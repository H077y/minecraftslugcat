...
function squapi.animate(
	...
	local runanim = runanim or walkanim
	...
	if idle ~= nil then animations.model.altidle:play() end
	...
	if rise ~= nil then rise:play() end
	if fall ~= nil then fall:play() end
	if crawling ~= nil then crawling:play() end
	if elytra ~= nil then elytra:play() end
	...
	function events.render(delta, context)
		...
		--Elytra
		if elytra ~= nil and pose == "FALL_FLYING" then
		...
		if fallimpact ~= nil then
			if yvel ~= 0 and grounded and not fallimpact:isPlaying() then
		...
		if fall ~= nil and rise ~= nil and pose ~= "FALL_FLYING" then
			if yvel < 0 then	
		...
		if sidewalkleft ~= nil and sidewalkright ~= nil and pose ~= "FALL_FLYING" and pose ~= "CROUCHING" then
			if svel > 0 then
		...
		-- Walk Animation and crawlmoving animation

		if walkanim ~= nil and pose ~= "FALL_FLYING" then
			local walk = walkanim
...
function squapi.smoothHead(element, tilt, keeporiginalheadpos)
	...
	function events.render(delta, context)
		...
		element:setOffsetRot(mainheadrot-offset)
...
-- *keeporiginalheadpos: when true(automatically true) the heads position will change like normally, set to false to disable.
function squapi.smoothHeadNeck(element, element2, tilt, keeporiginalheadpos)
	tilt = tilt or 2.5
	tilt = tilt/5
	if keeporiginalheadpos == nil then keeporiginalheadpos = true end
	local mainheadrot = vec(0, 0, 0)
	function events.render(delta, context)
		...
		mainheadrot[3] = mainheadrot[2]*tilt
		mainheadrot = mainheadrot
		element:setOffsetRot(mainheadrot * 0.6)
		element2:setOffsetRot(mainheadrot * 0.4)
		if keeporiginalheadpos then 
			element:setPos(-vanilla_model.HEAD:getOriginPos()) 
		end
